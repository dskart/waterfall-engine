...
// The type T must implement the Model interface and have a WithPatch method that takes a userId, patch and returns a pointer to the patched object.
func Patch[T any, P any](revisionUserId uuid.UUID, id uuid.UUID, patch P, get getFunc[T], commit commitFunc[T], opts ...func(*PatchOptions[T])) (newRevision, prevRevision *T, err error) {
	options := PatchOptions[T]{}
	for _, o := range opts {
		o(&options)
	}

	withPatchArgs := []reflect.Value{reflect.ValueOf(revisionUserId), reflect.ValueOf(patch)}
	for i := 0; i < MaxOptimisticWriteAttempts; i++ {
		...
		if existing == nil {
			...
			equalityTest.Elem().FieldByName("RevisionUserId").Set(existingValue.Elem().FieldByName("RevisionUserId"))
